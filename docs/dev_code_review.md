...
| 缺乏依赖注入 | **严重** | APIClient/SSEClient 无法单元测试 |
...
2. 修复 SSHTunnelManager 内存泄漏

**P1（强烈建议）**:
3. 拆分 ChatTabView.swift → 提取子组件
4. 为 APIClient/SSEClient 创建协议，支持依赖注入
...
nonisolated private func startReceiveLoop(from conn: NWConnection, to channel: Channel) {
    conn.receive(minimumIncompleteLength: 1, maximumLength: 16 * 1024) { data, _, isComplete, error in
        ...
        // ❌ 问题：递归调用捕获 self，未使用 weak self
        self.startReceiveLoop(from: conn, to: channel)
    }
}
```

**影响**: 当 SSH 隧道持续活跃时，SSHTunnelManager 实例无法被释放，造成内存泄漏。
...
| 未覆盖领域 | 重要性 | 建议 |
|-----------|--------|------|
| AppState 核心逻辑 | **高** | 需要 mock APIClient 才能测试 |
