...
- 已完成：补上了一轮 SSE 行为测试，覆盖 `session.updated`、`message.updated`、`message.part.updated`、`session.status` 等关键事件的状态变化。
- 已开始：`AppState` 到 store 的真正拆分已经落第一刀，流式消息 delta 的累积与消息 upsert 继续往 `MessageStore` 下沉。
- 仍待继续：`AppState` 的主体职责拆分和 `ChatTabView` 的组件化重构仍然是下一阶段的主体工作。
...
| 未覆盖领域 | 重要性 | 建议 |
|-----------|--------|------|
| AppState 核心逻辑 | **高** | 已补上关键 happy path / error path 与一轮 SSE 行为测试，后续继续覆盖恢复逻辑与更细的边界条件 |
...
3. **下一步把测试继续贴近 store split 切口**:
   - 优先补流式消息完成态、跨会话忽略、恢复同步相关场景
   - 在 `MessageStore` 开始承接更多 streaming helper 后，为它补更细粒度的单元测试
...
3. **创建专注的 Store**
   ...
   当前建议的最小切口：先沿着 `message.part.updated` / streaming message 这条链路继续把逻辑下沉到 `MessageStore`，等消息流式状态更稳定后，再拆 session status / activity 相关职责。
